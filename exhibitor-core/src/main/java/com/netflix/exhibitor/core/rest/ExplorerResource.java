...
public class ExplorerResource {
    ...
    @Path("uploadFile")
    @Consumes("application/json")
    public Response uploadPropertiesFile(String body) throws Exception {

        HashMap<String, String> request;
        ObjectMapper mapper = new ObjectMapper();
        request = mapper.readValue(body, HashMap.class);

        InputStream stream = new ByteArrayInputStream(request.get("file").getBytes());

        Properties props = new Properties();
        props.load(stream);

        String path = request.get("startPath") + "/" + request.get("fileName");
        uploadProperties(context.getExhibitor().getLocalConnection(), path, props);
    ...
    private void uploadProperties(CuratorFramework curatorFramework, String path, Properties properties) {
        ...
        try {
            ...
            for (Map.Entry<Object, Object> e : properties.entrySet()) {
                if (curatorFramework.checkExists().forPath(path + e.getKey()) == null) {
                    curatorFramework.create().creatingParentsIfNeeded().forPath(path + "/" + e.getKey(), ((String) e.getValue()).getBytes());
                }
            }
