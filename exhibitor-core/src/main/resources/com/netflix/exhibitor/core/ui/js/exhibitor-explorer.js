...
function performFileUpload() {
    ...
    var fileInput = document.getElementById('get-properties-file');
    var file = fileInput.files[0];
    var fr = new FileReader();

    var data = {};

    data.startPath = $('#get-properties-path').val();
...
function openUsageAddPropertiesFileDialog(path) {
    $('#get-properties-path').val(path);

    $("#get-properties-dialog").dialog("option", "buttons", {
            "Cancel": function () {
                $(this).dialog("close");
            },
    ...
    $("#get-properties-dialog").dialog("open");
}
...
function initExplorer() {
    ...
    $("#explorer-button-upload-properties").button({
        icons: {
            primary: "ui-icon-document"
        }
    }).click(function () {
        openUsageAddPropertiesFileDialog(explorerSelectedPath);
        return false;
    });
